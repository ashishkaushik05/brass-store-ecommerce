            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20">
                ...
                <div className="flex flex-col gap-4 sticky top-24 h-max">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        {(product.images || [product.imageUrl, ...products.slice(5, 8).map(p => p.imageUrl)]).slice(0,4).map((img, idx) => (
                ...
                <div className="flex flex-col h-full">
                    <div className="pb-6 border-b border-[#e6e2db]">
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <div className="flex items-center gap-1">
                                {[...Array(5)].map((_, i) => <Icon key={i} name="star" className="text-primary text-[18px]" style={{fontVariationSettings: "'FILL' 1"}}/>)}
                                <span className="text-sm text-text-subtle ml-1 underline cursor-pointer hover:text-primary">({reviews.length} Reviews)</span>
                            </div>
                        </div>
                        <p className="text-[#4a453e] text-base leading-relaxed">
                            Hand-beaten by master artisans, this piece captures golden hour light in its textured finish, bringing timeless elegance to your modern home.
                        </p>
                    </div>
                        ...
                        <div className="flex gap-4 items-end">
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex items-center h-12 border border-[#e6e2db] rounded-lg w-32 bg-white">
                                    ...
                                    <button onClick={() => setQuantity(q => q + 1)} className="flex-1 h-full flex items-center justify-center text-xl text-text-subtle hover:text-text-main transition-colors">+</button>
                                </div>
                            </div>
                            <button className="flex-1 h-12 bg-primary hover:bg-opacity-90 text-text-main font-bold rounded-lg uppercase tracking-wide transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                                <Icon name="shopping_bag" className="text-[20px]" /> Add to Cart
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="py-6">
                        <AccordionItem title="Material & Finish">
                            ...
                            <p><strong>Finish:</strong> {product.finish || 'Polished'}. Each piece is hand-finished, leading to unique variations that are a hallmark of artisanal craftsmanship.</p>
                        </AccordionItem>
                        <AccordionItem title="Dimensions & Weight">
                             <p>{product.dimensions || 'Dimensions not available'}</p>
                             <p>{product.weight || 'Weight not available'}</p>
